    ...
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<Usuario> Crear(Usuario entidad, Stream Foto = null, string NombreFoto = "", string UrlPlantillaCorreo = "")
        {
            ...
            try
            {
                ...
                Usuario usurario_creado = await _repositorio.Crear(entidad);

                if (usurario_creado.IdUsuario == 0)
                    throw new TaskCanceledException("No se puede crear el usuario");

                if (UrlPlantillaCorreo != "")
                {
                    UrlPlantillaCorreo = UrlPlantillaCorreo.Replace("[correo]", usurario_creado.Correo).Replace("[clave]", clave_generada);
                    ...
                    if (htmlCorreo != "")
                        await _correoService.EnviarCorreo(usurario_creado.Correo, "Cuenta Creada", htmlCorreo);

                }

            IQueryable<Usuario> query = await _repositorio.Consultar(u => u.IdUsuario == usurario_creado.IdUsuario);
            usurario_creado = query.Include(r => r.IdRolNavigation).First();

            return usurario_creado;
        }
